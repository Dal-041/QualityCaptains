...
public class CaptainsForceConcentration {
	...
	public static boolean USE_FULL_FLANKING = true;
...
//	}
	...
	public static class Level3 implements CharacterStatsSkillEffect {
		public void apply(MutableCharacterStatsAPI stats, String id, float level) {
			if(USE_FULL_FLANKING) stats.getDynamic().getMod(Stats.CAN_DEPLOY_LEFT_RIGHT_MOD).modifyFlat(id, 1f);
		}
		
		public void unapply(MutableCharacterStatsAPI stats, String id) {
			if(USE_FULL_FLANKING) stats.getDynamic().getMod(Stats.CAN_DEPLOY_LEFT_RIGHT_MOD).unmodifyFlat(id);
		}
		
		public String getEffectDescription(float level) {
			if(USE_FULL_FLANKING) return "Able to deploy ships of all size classes from the flanks in all combat scenarios";
			return "";
		}
